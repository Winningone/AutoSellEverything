...
--   - Account-wide or character-specific quality filter scope
--   - GUI with:
--       * enable toggle
--       * scope checkbox
--       * quality checkboxes
...
if AutoSellEverythingCharDB.sellQualities == nil then
  AutoSellEverythingCharDB.sellQualities = {
    [0] = true
  }
end

if AutoSellEverythingCharDB.thisCharacterOnly == nil then
  AutoSellEverythingCharDB.thisCharacterOnly = true
end
...
local GUIFrame = nil
local qualityChecks = {}
local thisCharacterOnlyCheck = nil
...
local function TableHasAnySetValue(tbl)
...
local function GetActiveSellQualities()
  if AutoSellEverythingCharDB.thisCharacterOnly then
    AutoSellEverythingCharDB.sellQualities = AutoSellEverythingCharDB.sellQualities or { [0] = true }
    return AutoSellEverythingCharDB.sellQualities
  end

  AutoSellEverythingDB.sellQualities = AutoSellEverythingDB.sellQualities or { [0] = true }
  return AutoSellEverythingDB.sellQualities
end

local function SetThisCharacterOnly(enabled)
  if enabled then
    if not AutoSellEverythingCharDB.thisCharacterOnly then
      if not TableHasAnySetValue(AutoSellEverythingCharDB.sellQualities) then
        AutoSellEverythingCharDB.sellQualities = CopyTableShallow(AutoSellEverythingDB.sellQualities or {})
      end
    end
    AutoSellEverythingCharDB.thisCharacterOnly = true
  else
    AutoSellEverythingCharDB.thisCharacterOnly = false
  end
end
...
local function IsSellQuality(quality)
  ...
  return GetActiveSellQualities()[quality] == true
end
...
local function SetAllQualities(enabled)
  local qualities = GetActiveSellQualities()
  for q = 0, 7 do
    qualities[q] = enabled and true or nil
  end
end
...
local function UpdateGUI()
  ...
  if statusText then
    local scopeLabel = AutoSellEverythingCharDB.thisCharacterOnly and "Character" or "Account"
    statusText:SetText("Auto-sell: " .. (AUTOSELL_ENABLED and "|cff00ff00Enabled|r" or "|cffff0000Disabled|r") .. "   Scope: " .. scopeLabel)
  end

  local activeQualities = GetActiveSellQualities()

  if thisCharacterOnlyCheck then
    thisCharacterOnlyCheck:SetChecked(AutoSellEverythingCharDB.thisCharacterOnly == true)
  end

  for q = 0, 7 do
    if qualityChecks[q] then
      qualityChecks[q]:SetChecked(activeQualities[q] == true)
    end
  end
...
local function CreateGUI()
  ...
  qualityHeader:SetText("Quality Filter Scope")

  local qualitySubHeader = GUIFrame:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
  qualitySubHeader:SetPoint("TOPLEFT", 18, -118)
  qualitySubHeader:SetText("Choose whether sell-quality filters apply to this character or the whole account")

  thisCharacterOnlyCheck = CreateFrame("CheckButton", nil, GUIFrame, "UICheckButtonTemplate")
  thisCharacterOnlyCheck:SetPoint("TOPLEFT", 18, -140)
  thisCharacterOnlyCheck:SetChecked(AutoSellEverythingCharDB.thisCharacterOnly == true)

  local thisCharacterOnlyLabel = GUIFrame:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
  thisCharacterOnlyLabel:SetPoint("LEFT", thisCharacterOnlyCheck, "RIGHT", 4, 1)
  thisCharacterOnlyLabel:SetText("This Character Only")

  thisCharacterOnlyCheck:SetScript("OnClick", function(self)
    SetThisCharacterOnly(self:GetChecked() and true or false)
    RefreshSellingState()
    UpdateGUI()
  end)

  local qualityListHeader = GUIFrame:CreateFontString(nil, "OVERLAY", "GameFontNormal")
  qualityListHeader:SetPoint("TOPLEFT", 18, -168)
  qualityListHeader:SetText("Sell Qualities")
  ...
  for q = 0, 7 do
    ...
    cb:SetPoint("TOPLEFT", 32 + (col * 210), yBase - (row * 26))
    cb:SetChecked(GetActiveSellQualities()[q] == true)
    cb.qualityID = q
    ...
    cb:SetScript("OnClick", function(self)
      ...
      if self:GetChecked() then
        qualities[self.qualityID] = true
      else
        qualities[self.qualityID] = nil
      end
      RefreshSellingState()
      UpdateGUI()
    end)
  ...
  allBtn:SetWidth(80)
  allBtn:SetHeight(22)
  allBtn:SetPoint("TOPLEFT", 20, -310)
  allBtn:SetText("All")
  ...
  tooltipCheck:SetPoint("TOPLEFT", 20, -338)
  tooltipCheck:SetChecked(AutoSellEverythingDB.showTooltipNote == true)
  ...
  ignoreEditBox:SetPoint("TOPLEFT", 20, -434)
  ...
  dragDropBox:SetPoint("TOPLEFT", 20, -466)
  ...
  globalHeader:SetPoint("TOPLEFT", 20, -508)
  ...
  globalIgnoreScrollFrame:SetPoint("TOPLEFT", 20, -528)
  ...
  characterHeader:SetPoint("TOPLEFT", 282, -508)
  ...
  characterIgnoreScrollFrame:SetPoint("TOPLEFT", 282, -528)
  ...
  clearGlobalBtn:SetPoint("TOPLEFT", 20, -638)
  ...
  clearCharacterBtn:SetPoint("TOPLEFT", 282, -638)
...
SlashCmdList["AUTOSELLSTATUS"] = function()
  local scopeLabel = AutoSellEverythingCharDB.thisCharacterOnly and "Character" or "Account"
  Print("Status: " .. (AUTOSELL_ENABLED and "enabled" or "disabled") .. ". Quality scope: " .. scopeLabel .. ".")
end
...
SlashCmdList["AUTOSELLQUALITY"] = function(msg)
  ...
  local function PrintQualityList()
    ...
    if #t == 0 then
      Print(scopeLabel .. " sell qualities: (none)")
    else
      Print(scopeLabel .. " sell qualities: " .. table.concat(t, " "))
    end
  end
...
SlashCmdList["AUTOSELLGUI"] = function()
  ToggleGUI()
end-- AutoSellEverything (WoW 3.3.5 - Wrath of the Lich King)
-- Features:
--   - Auto-sells while merchant window is open
--   - Sells newly created/moved items while merchant remains open
--   - Ignore list by item ID or item link
